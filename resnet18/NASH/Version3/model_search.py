...
class ResNet18(nn.Module):
  ...
  def genotype(self):
    ...
    def _parse_nm(group_arch_probabilities):
      gene = []
      n = 1 #//
      start = 0
      for i in range(self._steps):
        ...
        for j in edges:
          ...
          if j ==0 and (k_best == PRIMITIVES.index('skip_connect') or k_best == PRIMITIVES.index('max_pool_3x3')):
            W1 = W.copy()
            W1[j][k_best] = 0
            edges1 = sorted(range(i + 1), key=lambda x: -max(W1[x][k] for k in range(len(W1[x]))))[:1]
            for f in edges1:
              k_best1 = None
              ...
              if f ==0 and (k_best1 == PRIMITIVES.index('skip_connect') or k_best1 == PRIMITIVES.index('max_pool_3x3')):
                W2 = W1.copy()
                W2[f][k_best1] = 0
                edges2 = sorted(range(i + 1), key=lambda x: -max(W2[x][k] for k in range(len(W2[x]))))[:1]
                for q in edges:
                  k_best2 = None                  
    ...
    gene_g1 = _parse_nm(F.softmax(self.group1_arch_weights, dim=-1).data.cpu().numpy())
    gene_g2 = _parse_nm(F.softmax(self.group2_arch_weights, dim=-1).data.cpu().numpy())
    gene_g3 = _parse_nm(F.softmax(self.group3_arch_weights, dim=-1).data.cpu().numpy())
